...
/**
 * Resolves one attachment path through the same trust boundary used by the
 * provider builders.  The websocket gateway normally filters upload-store
 * descriptors before a runtime is called, but provider adapters are also
 * reachable from legacy/internal callers.  Checking the target's real path
 * here closes the symlink escape in that second path: a lexical path inside
 * the upload store (or cwd) must not resolve to a file outside either root.
 *
 * Missing files intentionally retain their lexical path.  Queued messages
 * can arrive before the upload has finished materializing; preserving that
 * path lets the provider report the ordinary missing-file error without
 * widening the readable filesystem.  A dangling symlink is treated the same
 * way because it cannot expose bytes until it resolves to an existing target,
 * at which point the provider's file read performs the normal check again.
 */
export function resolveAllowedImageSourcePath(
  resolvedPath: string,
  cwd?: string,
): string | null {
  ...
  try {
    const canonicalPath = path.resolve(realpathSync(resolvedPath));
    return isAllowedImageSourcePath(canonicalPath, cwd)
      ? canonicalPath
      : null;
  } catch {
    return resolvedPath;
...
/**
 * Applies the provider-side attachment trust boundary to descriptors that are
 * about to be serialized into a prompt.  The websocket gateway performs the
 * same check for browser-originated frames, but runtimes are also callable by
 * legacy/internal code.  Keeping this second check at the serialization
 * boundary prevents a direct caller from turning a `<*_input>` reference into
 * an arbitrary filesystem read.
 *
 * `cwd` is intentionally optional for backwards compatibility with history
 * adapters and tests, which use this helper only as a reversible prompt
 * formatter.  Every live provider runtime passes its resolved working
 * directory, so live descriptors are always checked before they reach a
 * child process/SDK.
 */
function resolveProviderAttachmentDescriptors(
  ...
  const resolved: ChatAttachmentDescriptor[] = [];
  for (const descriptor of attachments) {
    // Newlines would let a user-controlled filename forge additional numbered
    // entries or close the input tag early.  Uploaded paths are generated by
    // the server, but reject malformed legacy descriptors as a defense in
    // depth rather than interpolating them into a provider prompt.
    if (/[\r\n]/.test(descriptor.path)) {
      console.warn(`[Attachments] Refusing path containing a newline: ${descriptor.path}`);
      continue;
    }

    const resolvedPath = resolveImageAbsolutePath(cwd, descriptor.path);
    const allowedPath = resolveAllowedImageSourcePath(resolvedPath, cwd);
    if (!allowedPath) {
      console.warn(`[Attachments] Refusing attachment outside allowed roots: ${descriptor.path}`);
      continue;
    }

    resolved.push({ ...descriptor, path: allowedPath });
  }
  return resolved;
}
 ...
 */
export function appendImagesInputTag(prompt: string, images: unknown, cwd?: string): string {
  const descriptors = resolveProviderAttachmentDescriptors(
    normalizeImageDescriptors(images),
    cwd,
  );
 ...
 */
export function appendFilesInputTag(prompt: string, files: unknown, cwd?: string): string {
  const descriptors = resolveProviderAttachmentDescriptors(
    normalizeAttachmentDescriptors(files),
    cwd,
  );
  if (descriptors.length === 0) {
    return prompt;
